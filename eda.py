...
def synonym_replacement(words, n, synonym = True):
...
def random_deletion(words, p, stopwords = False):
...
def eda(sentence, alpha_sr=0, alpha_ri=0, alpha_rs=0, p_rd=0.1, num_aug=3, stopwords = False):
	...
	if (p_rd > 0):
		for _ in range(num_new_per_technique):
			a_words = random_deletion(words, p_rd, stopwords)
			augmented_sentences.append(' '.join(a_words))
